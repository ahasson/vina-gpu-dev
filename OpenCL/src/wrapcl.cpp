...
void SetupPlatform(cl_platform_id **platforms, cl_int *gpu_platform)
{
    ...
    *platforms = (cl_platform_id *)malloc(sizeof(cl_platform_id) * (num_platform));
    ...
    for (int i = 0; i < num_platform; i++)
    {
        ...
        err = clGetPlatformInfo((*platforms)[i], CL_PLATFORM_NAME, size, platform_name, NULL);
        checkErr(err);
        std::string tmp = platform_name;
        if (tmp.find(nvidia) != std::string::npos)
        {
            *gpu_platform = i;
            printf("GPU Platform: %s", platform_name);
            fflush(stdout);
            ...
            err = clGetPlatformInfo((*platforms)[i], CL_PLATFORM_VERSION, size, platform_version, NULL);
            checkErr(err);
    ...
    for (int i = 0; i < num_platform; i++)
    {
        ...
        {
            *gpu_platform = i;
            printf("GPU OpenCL Platform: %s", platform_name);
            fflush(stdout);
#ifdef DISPLAY_ADDITION_INFO
            err = clGetPlatformInfo((*platforms)[i], CL_PLATFORM_VERSION, 0, NULL, &size);
            checkErr(err);
...
void SetupDevice(cl_platform_id *platforms, cl_device_id **devices, cl_int gpu_platform)
{
    ...
    err = clGetDeviceIDs(platforms[N], CL_DEVICE_TYPE_CPU, 0, NULL, &num_device);
    checkErr(err);
    *devices = (cl_device_id *)malloc(sizeof(cl_device_id) * num_device);
    err = clGetDeviceIDs(platforms[N], CL_DEVICE_TYPE_CPU, num_device, *devices, NULL);
    checkErr(err);
#else
    err = clGetDeviceIDs(platforms[N], CL_DEVICE_TYPE_GPU, 0, NULL, &num_device);
    checkErr(err);
    *devices = (cl_device_id *)malloc(sizeof(cl_device_id) * num_device);
    err = clGetDeviceIDs(platforms[N], CL_DEVICE_TYPE_GPU, num_device, *devices, NULL);
    checkErr(err);
#endif
    // Display devices info
    for (int i = 0; i < num_device; i++)
    {
        err = clGetDeviceInfo((*devices)[i], CL_DEVICE_NAME, 0, NULL, &device_name_size);
        checkErr(err);
...
void SetupContext(cl_platform_id *platforms, cl_device_id *devices, cl_context *context, cl_uint num_device, cl_int gpu_platform_id)
{
    ...
    *context = clCreateContext(properties, num_device, devices, NULL, NULL, &err);
    ...
    err = clGetContextInfo(*context, CL_CONTEXT_NUM_DEVICES, sizeof(cl_int), &num_device_in_context, NULL);
    checkErr(err);
...
void SetupQueue(cl_command_queue *queue, cl_context context, cl_device_id *devices)
{
    ...
    // choose 0th device
    *queue = clCreateCommandQueue(context, devices[0], *props, &err);
    checkErr(err);
...
void SetupBuildProgramWithSource(cl_program program_cl, cl_program program_head, cl_device_id *devices, std::string include_path, std::string addtion)
{
    ...
    if (CL_SUCCESS != err)
    {
        ...
        size_t logsize;
        // Building log
        err = clGetProgramBuildInfo(program_cl, *devices, CL_PROGRAM_BUILD_LOG, 0, NULL, &logsize);
        checkErr(err);
        buffer = (char *)malloc(logsize * sizeof(char));
        err = clGetProgramBuildInfo(program_cl, *devices, CL_PROGRAM_BUILD_LOG, logsize, buffer, NULL);
        checkErr(err);
        printf("\nlog:%s", buffer);
        free(buffer);
        exit(-1);
...
void SaveProgramToBinary(cl_program program_cl, const char *file_name)
{
    ...
    err = clGetProgramInfo(program_cl, CL_PROGRAM_NUM_DEVICES, sizeof(cl_uint), &numDevices, NULL);
    checkErr(err);
    cl_device_id *devices = (cl_device_id *)malloc(sizeof(cl_device_id) * numDevices);
    err = clGetProgramInfo(program_cl, CL_PROGRAM_DEVICES, sizeof(cl_device_id) * numDevices, devices, NULL);
    checkErr(err);
    size_t *programBinarySize = (size_t *)malloc(sizeof(size_t) * numDevices);
    err = clGetProgramInfo(program_cl, CL_PROGRAM_BINARY_SIZES, sizeof(size_t) * numDevices, programBinarySize, NULL);
    checkErr(err);
    ...
    err = clGetProgramInfo(program_cl, CL_PROGRAM_BINARIES, sizeof(unsigned char *) * numDevices, programBinaries, NULL);
    checkErr(err);
...
cl_program SetupBuildProgramWithBinary(cl_context context, cl_device_id *devices, const char *binary_file_name)
{
    ...
    cl_program program_cl = clCreateProgramWithBinary(context, 1, devices, &program_size, (const unsigned char **)&binary_buffer, &binary_status, &err);
    checkErr(err);
...
void SetupKernel(cl_kernel *kernels, cl_program *programs, size_t num_kernel, const char kernel_name[][50])
{
    ...
    {
        kernels[i] = clCreateKernel(programs[i], kernel_name[i], &err);
        checkErr(err);
    }
}
...
{
    cl_int err;
    *mem = clCreateBuffer(context, flag, size, NULL, &err);
    checkErr(err);
}
