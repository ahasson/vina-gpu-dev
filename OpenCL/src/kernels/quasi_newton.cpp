...
inline int num_atom_types(int atu)
{
	...
	case 3:
		return SY_TYPE_SIZE;
	default:
		printf("Kernel1:num_atom_types() ERROR!");
		return INFINITY;
	}
}
					...
					const float epsilon_fl)
{
	...
	for (int i = 0; i < m->m_num_movable_atoms; i++)
	{
		...
		for (int j = 0; j < 3; j++)
			m->minus_forces.coords[i][j] = deriv[j];
	}
	return e;
}
		 ...
		 const float epsilon_fl)
{
	...
	{ // current starts from 1 (namely starts from first child node)
		...
		local_to_lab(lig_rigid_gpu->origin[current],
					 ...
					 lig_rigid_gpu->orientation_m[parent]); // set origin
		local_to_lab_direction(lig_rigid_gpu->axis[current],
							   lig_rigid_gpu->relative_axis[current],
							   lig_rigid_gpu->orientation_m[parent]); // set axis
		float tmp[4];
			   ...
			   change_cl *g)
{
	...
	{ // from bottom to top
		for (int k = 0; k < 3; k++)
			orientation_derivative[i][k] = orientation_derivative_tmp[i][k]; // self
		// looking for chidren node
		for (int j = 0; j < num_rigid; j++)
		{
			if (lig_rigid_gpu->children_map[i][j] == true)
	...
	for (int k = 0; k < num_torsion; k++)
		g->lig_torsion[k] = torsion_derivative[k + 1]; // no meaning for node 0
}
				 ...
				 const int torsion_size)
{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = i; j < n; j++)
		{
			float tmp = alpha * r * (find_change_index_read(&minus_hy, i, s) * find_change_index_read(p, j, s) + find_change_index_read(&minus_hy, j, s) * find_change_index_read(p, i, s)) +
