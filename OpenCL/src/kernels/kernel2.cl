void get_heavy_atom_movable_coords(output_type_cl *tmp, const m_cl *m,
  ...
  for (int i = 0; i < m->m_num_movable_atoms; i++) {
    if (m->atoms[i].types[0] != EL_TYPE_H) {
      ...
      // printf("\n kernel2: removed H atom coords in
...
__kernel void kernel2(const __global output_type_cl *ric, __global m_cl *mg,
                      __constant pre_cl *pre, __constant grids_cl *grids,
                      __constant random_maps *random_maps,
                      __global ligand_atom_coords_cl *coords,
                      __global output_type_cl *results,
                      const __global mis_cl *mis, const int torsion_size,
                      const int search_depth, const int max_bfgs_steps) {
  ...
  for (int gll = gl; gll < mis->thread; gll += mis->total_wi) {
    ...
    for (int step = 0; step < search_depth; step++) {
      ...
      mutate_conf_cl(map_index, &candidate, random_maps->int_map,
                     random_maps->sphere_map, random_maps->pi_map,
                     m.ligand.begin, m.ligand.end, m.atoms, &m.m_coords,
                     m.ligand.rigid.origin[0], mis->epsilon_fl,
                     mis->mutation_amplitude, torsion_size);
      bfgs(&candidate, &g, &m, pre, grids, mis, torsion_size, max_bfgs_steps);
      ...
      if (step == 0 || metropolis_accept(tmp.e, candidate.e, 1.2, n)) {
        ...
        set(&tmp, &m.ligand.rigid, &m.m_coords, m.atoms, m.m_num_movable_atoms,
            mis->epsilon_fl);

        if (tmp.e < best_e) {
          bfgs(&tmp, &g, &m, pre, grids, mis, torsion_size, max_bfgs_steps);
          // set
          if (tmp.e < best_e) {
            set(&tmp, &m.ligand.rigid, &m.m_coords, m.atoms,
                m.m_num_movable_atoms, mis->epsilon_fl);

            best_out = tmp;
            get_heavy_atom_movable_coords(&best_out, &m,
                                          &best_coords); // get coords
            best_e = tmp.e;
          }
        }
      }
