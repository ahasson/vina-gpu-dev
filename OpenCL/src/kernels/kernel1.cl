...
float eval_fast(int type_pair_index, float r2, float cutoff_sqr,
...
const __global int *possibilities(float *coords, const __global ar_cl *ar,
                                  const float epsilon_fl,
                                  const __global gb_cl *gb, int *relation_count,
  ...
  for (int i = 0; i < 3; i++) {
    if (coords[i] + epsilon_fl < gb->init[i])
      printf("\nkernel1:possibilities ERROR!1 coords = %f,  init = %f",
             coords[i], gb->init[i]); // replace assert()
    if (coords[i] > gb->init[i] + gb->range[i] + epsilon_fl)
      printf(
          "\nkernel1:possibilities ERROR!2 coords = %f,  init = %f, range =%f",
          coords[i], gb->init[i], gb->range[i]); // replace assert()
...
__kernel void kernel1(const __global pre_cl *pre, const __global pa_cl *pa,
                      const __global gb_cl *gb, const __global ar_cl *ar,
                      __global grids_cl *grids, const __global mis_cl *mis,
                      const __global float *needed, const int atu,
                      const int nat) {
  ...
  float probe_coords[3] = {grids->grids[grids_front].m_init[0] +
                               grids->grids[grids_front].m_factor_inv[0] * x,
                           grids->grids[grids_front].m_init[1] +
                               grids->grids[grids_front].m_factor_inv[1] * y,
                           grids->grids[grids_front].m_init[2] +
                               grids->grids[grids_front].m_factor_inv[2] * z};
  int relation_count;

  const __global int *possibilities_ptr = possibilities(
      probe_coords, ar, mis->epsilon_fl, gb, &relation_count, mis);

  for (int possibilities_i = 0; possibilities_i < relation_count;
       possibilities_i++) {
    ...
    if (r2 <= mis->cutoff_sqr) {
      for (int j = 0; j < mis->needed_size; j++) {
        ...
        const int type_pair_index =
            triangular_matrix_index_permissive(num_atom_types(atu), t1, t2);
